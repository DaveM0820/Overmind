...
public class UnitCommand : MonoBehaviour
{
    ...
    public void GiveMoveOrder(Vector3 location) //gives move order to all selected units
    {
        ...
        Debug.Log("gave move order GiveMoveOrder");
    }
    ...
    public void GiveOneMoveOrder(GameObject unit, Vector3 location, bool emptyQueue = true)//use astar pathfinding to create a path for that unit
    {
        ...
        // Start a new path request from the current position to a position 10 units forward.
        // When the path has been calculated, it will be returned to the function OnPathComplete unless it was canceled by another path request
        seeker.StartPath(unit.transform.position, location, unit.GetComponent<UnitBehaviour>().OnPathComplete);//run OnPathComplete in that unit's Unitbehaviour which will convert the vector3 array to move orders in the unit's order queue
        Debug.Log("gave move order GiveOneMoveOrder");
    }
